...
void WiFiManager::handleWifi(boolean scan) {
        ...
        for (int i = 0; i < n; i++) {
          ...
          for (int j = i + 1; j < n; j++) {
            if (cssid == WiFi.SSID(indices[j])) {
              DEBUG_WM(String(F("DUP AP: ")) + WiFi.SSID(indices[j]));
  ...
  for (int i = 0; i < _paramsCount; i++) {
    ...
    if (_params[i]->getID() != NULL) {
      pitem.replace(F("{i}"), _params[i]->getID());
      pitem.replace(F("{n}"), _params[i]->getID());
      pitem.replace(F("{p}"), _params[i]->getPlaceholder());
      snprintf(parLength, 2, "%d", _params[i]->getValueLength());
      pitem.replace(F("{l}"), parLength);
      pitem.replace(F("{v}"), _params[i]->getValue());
      pitem.replace(F("{c}"), _params[i]->getCustomHTML());
    } else {
